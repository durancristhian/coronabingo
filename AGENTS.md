...
- Vercel Preview gameplay verification is owner-approved, including when the Preview uses the Production Firestore project. Create only short-lived, task-owned game records with unmistakable test names. Record their IDs and delete them after verification. This standing approval covers gameplay records only; changes to persistent content, Firebase rules or configuration, billing, and Production deployments still require separate authorization.
