...
- Bootstrap every new worktree before implementation or analysis begins. Copy `.env` from the local checkout used to create it into the new worktree at the same relative path. This instruction is owner approval for that local copy when both Coronabingo checkouts target the same environment. Never print or commit its contents, and never overwrite an existing destination `.env`; resolve a missing source or existing destination before continuing.
- In the new worktree, activate the runtime from `.nvmrc` and run `npm ci`. Setup is complete only after the install succeeds and the reported Node and npm versions satisfy `.nvmrc` and `package.json`. Each checkout owns its `node_modules`, `.next`, and TypeScript build cache; keep generated state separate between worktrees.
- Follow the environment rules below when setting up a new checkout. Copying `.env` does not isolate the shared Firebase backend.
...
- Before a browser flow writes data, verify the Firebase target. Localhost and Vercel Preview can connect to Production; their URL alone does not establish data isolation.
- Vercel Preview gameplay verification is owner-approved, including when the Preview uses the Production Firestore project. Create only short-lived, task-owned game records with unmistakable test names and record their IDs. After verification, delete their player records and delete the room when the current rules and authorized tooling permit it. If room deletion is blocked, leave it empty, record its ID and report the cleanup gap. This standing approval covers gameplay records only; changes to other persistent content, Firebase rules or configuration, billing, and Production deployments still require separate authorization.
...
- Record test data created and clean up only data owned by the task when that cleanup is authorized. Except for the standing Vercel Preview gameplay approval above, changes to Production data, Firebase rules, billing, or provider configuration require explicit authorization for the target and action.
