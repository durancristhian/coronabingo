...
PERF-04 extended the existing host/player journey. Before saving, the host leaves setup through browser history and returns with all three local participant drafts intact, then removes the temporary participant. After both remaining participants open their assigned cards, the host returns to the room through browser history, sees both participant rows, moves forward again and recovers the same assigned cards and host controls. The journey also observes the same sound and celebration option in both browser contexts, changes to a second room without exposing the first room's players, returns through browser history, deletes a persisted player while that player's card view remains open, and retains the existing number, reload and restart checks.

`npm run ui-tests -- tests/ui/room.spec.ts` passed in development. The final `npm run ui-tests:production` generated an isolated build and passed against a freshly owned Firestore Emulator at `demo-coronabingo-ui`; Playwright reported one passing test in 4.0 seconds and the complete runner took 11.4 seconds. Sound playback was intercepted to assert propagation without relying on an audio device. An earlier production browser run, before the final draft-preservation correction, was discarded because a concurrent worktree acquired the fixed emulator ports during its build. No hosted Firebase data, Preview or Production environment was exercised. Details remain in [PERF-04's evidence](performance/perf04-evidence/README.md).
